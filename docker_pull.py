...
def bla(repository, registry, tag, img, imgparts, repo):
    ...
    if (resp.status_code != 200):
        ...
        print(resp.content)
        auth_head = get_auth_head('application/vnd.docker.distribution.manifest.list.v2+json', repository)
        resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry,
                            repository, tag), headers=auth_head, verify=False)
        if (resp.status_code == 200):
    ...
    confresp = requests.get('https://{}/v2/{}/blobs/{}'.format(registry,
                            repository, config), headers=auth_head, verify=False)
    ...
    for layer in layers:
        ...
        auth_head = get_auth_head('application/vnd.docker.distribution.manifest.v2+json', repository)
        bresp = requests.get('https://{}/v2/{}/blobs/{}'.format(registry, repository, ublob),
        ...
        if (bresp.status_code != 200):  # When the layer is located at a custom URL
            ...
            if (bresp.status_code != 200):
                print('\rERROR: Cannot download layer {} [HTTP {}]'.format(
